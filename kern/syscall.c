...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if((perm & PTE_U) == 0 || (perm & PTE_P) == 0 || (perm & ~(PTE_U | PTE_P | PTE_AVAIL | PTE_W)) != 0){
		return -E_INVAL;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if(err_src < 0 || dst_env < 0){
		return -E_BAD_ENV;
	}
	
	if(((uintptr_t)srcva >= UTOP) || ((uintptr_t)srcva & (PGSIZE-1))
	 || ((uintptr_t)dstva >= UTOP) || ((uintptr_t)dstva & (PGSIZE-1))){
		return -E_INVAL;
	}
	...
	pte_t *src_pte = NULL;
	struct PageInfo * src_page = page_lookup(src_env->env_pgdir,srcva,&src_pte);
	if(NULL == src_page){
		return -E_INVAL;
	}
	...
	err_dst = page_insert(dst_env->env_pgdir,src_page,dstva,perm);
	if(err_dst < 0){
		return -E_NO_MEM;
	}
	return 0;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if((uintptr_t)srcva < UTOP){
		...
		if(!(page = page_lookup(curenv->env_pgdir,srcva,&pte_s))){
			DEBUG_LOG("\n");
			return -E_INVAL;
		}
		
		if((perm & PTE_W) && !((*pte_s) & PTE_W)){
			DEBUG_LOG("\n");
			return -E_INVAL;
		}
		if((err = page_insert(e->env_pgdir,page,e->env_ipc_dstva,perm)) < 0){
			return -E_NO_MEM;
		}
	}
